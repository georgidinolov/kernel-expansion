...
bivariate.solution.classical <- function(dx, dy, problem.parameters,
    ...
    par(mar=c(4,4,1,1))
    plot(boundary.points.mat[1,],
         boundary.points.mat[2,],
         col="red",
    ...
    border.direction <- list(c(1,0), c(0,1), c(-1,0), c(0,-1))
    Cs <- rep(NA, 4)
    for (border.index in seq(1,4)) {
        border.vector <- as.vector(T.mat %*% border.direction[[border.index]]);
        orthogonal.border.vector <- c(border.vector[2], -border.vector[1]);

        ## plotting border border.index
        lines(c(boundary.points.mat[1,border.index]+2*border.vector[1],
                boundary.points.mat[1,border.index]-2*border.vector[1]),
              c(boundary.points.mat[2,border.index]+2*border.vector[2],
                boundary.points.mat[2,border.index]-2*border.vector[2]))

        projection <- ((boundary.points.mat[1,border.index]-xi.ic)*
                       orthogonal.border.vector[1]+
                       (boundary.points.mat[2,border.index]-eta.ic)*
                       orthogonal.border.vector[2]) /
            sqrt(orthogonal.border.vector[1]^2 + orthogonal.border.vector[2]^2)

        ## plotting the projection length along green
        lines(c(xi.ic,
                xi.ic + 2*projection*orthogonal.border.vector[1]/
                sqrt(orthogonal.border.vector[1]^2 + orthogonal.border.vector[2]^2)),
              c(eta.ic,
                eta.ic + 2*projection*orthogonal.border.vector[2]/
                sqrt(orthogonal.border.vector[1]^2 + orthogonal.border.vector[2]^2)),
              col = "blue")
        Cs[border.index] = projection
    }
    dev.off()
    ...
    ##     (sin(ss.1) -p cos(ss.1)*sqrt(1-rho)/sqrt(1+rho));
    border.vector.1 <- T.mat %*% matrix(data=c(1,0), nrow=2, ncol=1)
    orthogonal.border.vector.1 <- c(border.vector.1[2],
                                    -border.vector.1[1])
    C.1 <- ((xi.ic-boundary.points.mat[1,1])*border.vector.1[2] -
            (eta.ic-boundary.points.mat[2,1])*border.vector.1[1]) /
        norm(border.vector.1)
    ...
    plot(c(boundary.points.mat[1,1], boundary.points.mat[1,2]),
         ...
         xlab = expression(xi),
         ylab = expression(eta));
    lines(c(boundary.points.mat[1,1],
            boundary.points.mat[1,1] + border.vector.1[1]),
          c(boundary.points.mat[2,1],
            boundary.points.mat[2,1] + border.vector.1[2]),
          col = "red")

    lines(c(xi.ic,
            xi.ic + orthogonal.border.vector.1[1]),
          c(eta.ic,
            eta.ic + orthogonal.border.vector.1[2]),
          col = "green")
    lines(c(xi.ic,
            xi.ic + (boundary.points.mat[1,1]-xi.ic)),
          c(eta.ic,
            eta.ic + (boundary.points.mat[2,1]-eta.ic)),
          col = "purple")
    ## projecting purple on green
    projection <- ((boundary.points.mat[1,1]-xi.ic)*orthogonal.border.vector.1[1]+
                   (boundary.points.mat[2,1]-eta.ic)*orthogonal.border.vector.1[2]) /
        sqrt(orthogonal.border.vector.1[1]^2 + orthogonal.border.vector.1[2]^2)
    ## plotting the projection length along green
    lines(c(xi.ic,
            xi.ic + projection*orthogonal.border.vector.1[1]/
            sqrt(orthogonal.border.vector.1[1]^2 + orthogonal.border.vector.1[2]^2)),
          c(eta.ic,
            eta.ic + projection*orthogonal.border.vector.1[2]/
            sqrt(orthogonal.border.vector.1[1]^2 + orthogonal.border.vector.1[2]^2)),
          col = "blue")

    points(boundary.points.mat[1,],
           boundary.points.mat[2,],
           col="red")
    points(c(xi.ic), c(eta.ic), col="red")
    ...
    border.vector <- T.mat %*% matrix(data=c(0,1), nrow=2, ncol=1);
    C.2 <- ((boundary.points.mat[1,2]-xi.ic)*-border.vector[2] +
            (boundary.points.mat[2,2]-eta.ic)*border.vector[1]) /
        norm(border.vector)
    ...
    border.vector <- T.mat %*% matrix(data=c(0,1), nrow=2, ncol=1);
    C.4 <- ((boundary.points.mat[1,4]-xi.ic)*-border.vector[2] +
            (boundary.points.mat[2,4]-eta.ic)*border.vector[1]) /
        norm(border.vector)
    ...
    border.vector <- T.mat %*% matrix(data=c(1,0), nrow=2, ncol=1);
    C.3 <- ((boundary.points.mat[1,3]-xi.ic)*-border.vector[2] +
            (boundary.points.mat[2,3]-eta.ic)*border.vector[1]) /
        norm(border.vector)
    ...
    if (PLOT.SOLUTION) {
        ...
        lines(c(xi.ic, C.1*-border.vector.1[2]),
              c(eta.ic, C.1*border.vector.1[1]),
              col="red");
