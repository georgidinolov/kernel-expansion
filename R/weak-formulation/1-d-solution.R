...
basis.function.init <- function(means,
                                variances,
                                t.0s,
                                i) {
    ## length of all input vectors must be the same!
    current.basis.function <- function(x,t) {
        return (dbeta(x,2,2)*dnorm(x,mean=means[i],
                                   sd=sqrt(variances[i]*(t+t.0s[i]))));
    }
    return (current.basis.function);
}

basis.function.init.dt <- function(means,variances,t.0s,i) {
    current.basis.function.dt <- function(x,t) {
        return (dbeta(x,2,2)*
                dnorm(x,mean=means[i],
                      sd=sqrt(variances[i]*(t+t.0s[i])))*
                ((x-means[i])^2/(2*sqrt(2*pi)*variances[i]*
                                 (t+t.0s[i])^2*
                                 sqrt(variances[i]*(t+t.0s[i]))) -
                 variances[i]/(2*sqrt(2*pi)*(variances[i]*(t+t.0s[i]))^(3/2))));
    }
    return (current.basis.function.dt);
}
