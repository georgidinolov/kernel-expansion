...
\begin{align}
  ...
  \MoveEqLeft[10] -\mu_x \frac{\partial}{\partial x}q(x,y,t')
  - \mu_y \frac{\partial}{\partial y}q(x,y,t') \nonumber \\
  \MoveEqLeft[10] + \frac{1}{2}\sigma_x^2 \frac{\partial^2}{\partial x^2}q(x,y,t')
...
\end{align}
decomposes the solution, preserves the initial and boundary
conditions, and eliminates the advection terms in the differential
operator. $T_{(0)}$ is defined as
\begin{align}
...
\begin{equation}
  \frac{\partial\, p}{\partial t} = \frac{1}{2}\sigma_x^2 \frac{\partial^2\, p}{\partial x^2} + \rho\sigma_x\sigma_y \frac{\partial^2\, p}{\partial x \partial y} + \frac{1}{2}\sigma_y^2 \frac{\partial^2\, p}{\partial y^2},\quad (x,y) \in (a_x,b_x) \times (a_y,b_y) := \Omega \label{eq:qq}
\end{equation}
subject to the constraints
\begin{align}
  p(x,y,t) &=0,\mbox{ for } (x,y) \in \{ x | x = a_x\} \cup \{ y | y = a_y\} \cup \{ x | x = b_x\} \cup \{ y | y = b_y\}, \nonumber \\
  p(x,y,0) &= \delta\left( x - x_0 \right) \delta\left(y-y_0\right), \nonumber
\end{align}
The problem is then \textit{standardized} with the scaling  
\begin{align}
...
$p(x^{(1)}, y^{(1)}, t)$
\begin{align}
  \frac{\partial}{\partial t}p(x^{(1)}, y^{(1)}, t) = \mathcal{L}^{(1)}p(x^{(1)}, y^{(1)}, t),\quad (x^{(1)},y^{(1)}) \in \Omega^{(1)} := (0,1) \times (0,1). \label{eq:qq-2}
\end{align}
...
\begin{remark} \label{remark:const-L} We consider $L_x$ and $L_y$ as
  \textit{fixed}. This amounts to perturbing the boundaries of
  $\Omega^{(1)}$ when differentiating $p(x^{(1)}, y^{(1)}, t)$ with
  respect to $(a_x, b_x, a_y, b_y)$, while keeping the diffusion
  parameters $\tau_x, \tau_y$ constant. Likewise, if we treat
...
\begin{align}
  T_{(2)}: p(x^{(1)},y^{(1)},t) &\to p(\tilde{x}, \tilde{y}, \tilde{t}),
\end{align}
...
\begin{align}
  \frac{\partial}{\partial \tilde{t}} p(\tilde{x},\tilde{y},\tilde{t}) &= \tilde{\mathcal{L}} p(\tilde{x},\tilde{y},\tilde{y}), & (\tilde{x}, \tilde{y}) \in (0,1) \times (0,1) := \tilde{\Omega} \label{eq:qqq} \\
  \tilde{\mathcal{L}} &= \frac{1}{2} \frac{\partial^2}{\partial \tilde{x}^2} + \rho \sigma_{\tilde{y}} \frac{\partial^2}{\partial \tilde{x} \partial \tilde{y}} + \frac{1}{2} \sigma^2_{\tilde{y}} \frac{\partial^2}{\partial \tilde{y}^2}, \label{eq:qqq-2}
\end{align}
with
...
\begin{align}
  ...
  p(\tilde{x},\tilde{y},0) &= \delta\left( \tilde{x} - \tilde{x}_0 \right)\delta\left( \tilde{y} - \tilde{y}_0 \right).
\end{align}
...
\[
  p(x,y,t) = p(\tilde{x}, \tilde{y}, \tilde{t}) \cdot \frac{1}{L_xL_y} \cdot \max\left\{\tau_x^2, \tau_y^2 \right\},
\]
such that the solution to the original problem \eqref{eq:1} -
\eqref{eq:2} can be expressed as the product
\[
  q(x,y,t) = \exp(-\alpha x - \beta y - \gamma t) \cdot p(\tilde{x}, \tilde{y}, \tilde{t}) \cdot \frac{1}{L_xL_y} \cdot \max\left\{\tau_x^2, \tau_y^2 \right\},
\]
...
density we are interested in, we must take derivatives of the solution
$p(\tilde{x},\tilde{y},\tilde{t})$ with respect to the boundary parameters
$(a_x, b_x, a_y, b_y)$. Because the approximate solutions we will
consider below are functions of the parameters without explicit
analytic form,
$\frac{\partial^4}{\partial a_x\partial b_x \partial a_y \partial
  b_y}p(\tilde{x},\tilde{y},\tilde{t})$ must be computed numerically using a finite difference
...
Consider an approximate solution $p^{(k)}(\left. \tilde{x},\tilde{y},\tilde{t} \right| b)$
...
$\mathbf{p}(0)$ is the vector projection of $p(\tilde{x},\tilde{y},0)$ onto the
...
\]
with $\boldsymbol{\psi}(\tilde{x},\tilde{y}) = (\psi_0(\tilde{x},\tilde{y}), \ldots, \psi_k(\tilde{x},\tilde{y}))^T.$
Since both $M$ and $S$ are symmetric and infinitely differentiable
with respect $(a_x, b_x, a_y, b_y)$ (each entry in the matrices has
...
differentiable with respect to the parameters $(a_x, b_x, a_y, b_y)$
...
\item Scale and rotate the coordinate axes by the transformation
  ...
  $\Omega^{(3)}$ as the shortest distance between $(\xi_0, \eta_0)$
...
\tilde{t}_\epsilon$. This replaces initial condition orthogonality
condition in (\ref{eq:orthogonality-conditions-mat-2}) with
\begin{align}
...
$R_e(k)$ and $R_0(k)$ will be, and the better our method will do.
...
\begin{align}
  \psi_i(\tilde{x},\tilde{y}) &= \frac{1}{2\pi \sigma^2\sqrt{1-\tilde{\rho}^2} } \\
                              &\quad \times \exp\left\{ -\frac{1}{2(1-\tilde{\rho}^2)\sigma^2} \left( (\tilde{x} - \tilde{x}_i)^2 - 2\tilde{\rho} (\tilde{x}-\tilde{x}_i)(\tilde{y}-\tilde{y}_i) + (\tilde{y} - \tilde{y}_i)^2 \right) \right\} \nonumber \\
...
a grid $\{ (\tilde{x}'_j, \tilde{y}'_j) \}_{j=0}^{k'}$ such that
$(\tilde{x}'_0, \tilde{y}'_0) = (1/2,1/2)$ and the subsequent grid
points are $l\sigma(1 + \tilde{\rho})$ units apart in the
$x-$direction and $l\sigma(1 - \tilde{\rho})$ units apart in the
$y-$direction over
...
problem. More basis elements are added by decreasing $\sigma$ and
altering $l$, while the degree of correlation is controlled by
$\tilde{\rho}$. For the purposes of this paper, we found that keeping
fixed $l=1$ with a moderate $\tilde{\rho}$ ($|\tilde{\rho}| \sim 0.6$)
...
approximation $p^{(k)}(\tilde{x},\tilde{y},\tilde{t})$ to the governing Fokker-Planck equation
(\ref{eq:1}) is consistent. To do so, we first show that the
distribution on $Z$ based on the approximate $p^{(k)}(\tilde{x},\tilde{y},\tilde{t})$ converges to
...
\begin{proof}
  ...
  with respect to the data parameter $x$, the estimator
  $\hat{\theta}_{n,k}(x)$ is a continuous function with respect to $x$
