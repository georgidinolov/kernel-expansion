...
We consider two-dimensional correlated Brownian motion:
\begin{align}
...
\begin{align}
  \displaystyle \frac{\partial}{\partial t'} q(x,y,t') = \nonumber \\
  \MoveEqLeft[16] -\mu_x \frac{\partial}{\partial x}q(x,y,t')
  - \mu_y \frac{\partial}{\partial y}q(x,y,t') + \frac{1}{2}\sigma_x^2 \frac{\partial^2}{\partial x^2}q(x,y,t')
...
\textit{some} of the observed maxima/minima. The authors in
\cite{rodriguez2012} use a full likelihood-based (Bayesian) approach
to estimate volatility in \textit{univariate} financial timeseries
where open, closing, highest, and lowest prices are included. Their
work fits into a body of literature and collection of techniques by
practitioners where the observed range of prices is used to make
similar estimates. In this chapter we will provide an efficient
numerical method necessary for carrying out inferential procedures
with correlated financial timeseries in the bivariate setting.
...
density function for just a single observation (see Section
...
nonzero $\rho$, the convergence is slow; it is necessary to have a
large number of basis elements for an accurate approximation. The
denseness and size of the resultant system makes the expansion a slow,
if not unfeasible, solution. An alternative is to use a finite
difference scheme to directly solve the evolution problem after
suitable transformations. However, both of these methods need a high
degree of numerical resolution to produce practically useful
approximations of the transition density. The trigonometric series
expansion has exponential accuracy in the case $\rho=0$ while it loses
accuracy catastrophically when $\rho\neq 0$. This fact suggests that
for the trigonometric series expansion inefficiencies arise from using
...
This chapter describes the Galkerin discretization. The key strategy
is to directly take into account the correlation parameter present in
the differential operator in order to efficiently represent an
...
\end{align}
The problem is then \textit{standardized} with the scaling transformation
\begin{align}
  T_{(1)}: & \left\{ \begin{array}{ccc}
                       p(x^{(1)}, y^{(1)}, t) &=& L_xL_y p(x,y,t), \\
                       p(x^{(1)}, y^{(1)}, 0) &=& \delta(x^{(1)}-x_0^{(1)}) \delta(y^{(1)}-y_0^{(1)}), \\
...
$p(x^{(1)}, y^{(1)}, t)$
\begin{align}
  \frac{\partial}{\partial t}p(x^{(1)}, y^{(1)}, t) &= \mathcal{L}^{(1)}p(x^{(1)}, y^{(1)}, t),\quad (x^{(1)},y^{(1)}) \in \Omega^{(1)}, \label{eq:qq-2} \\
  \Omega &:= (0,1) \times (0,1). \nonumber 
\end{align}
...
\begin{remark} \label{remark:const-L} We consider $L_x$ and $L_y$ as
  ...
  $p(x^{(1)}, y^{(1)}, t)$ with respect to $(a_x, b_x, a_y, b_y)$,
...
Next, we \textit{normalize} the diffusion equation (\ref{eq:qq-2}) with the transformation
\begin{align}
  T_{(2)}: p(\tilde{x}, \tilde{y}, \tilde{t}) &= \left\{ \begin{array}{cc}
                                                  p(x^{(1)}, y^{(1)}, \tilde{t}/\tau_x^2), &\mbox{if } \tau_x^2 \geq \tau_y^2 \\
                                                 p(y^{(1)}, x^{(1)}, \tilde{t}/\tau_y^2), &\mbox{otherwise},
                                                  \end{array} \right., \label{eq:T2}
\end{align}
where the function $p(\cdot)$ in the left-hand side of \eqref{eq:T2}
solves \eqref{eq:qq-2}, defined with the coordinate transformation
\begin{align}
...
\begin{align}
  \frac{\partial}{\partial \tilde{t}} p(\tilde{x},\tilde{y},\tilde{t}) &= \tilde{\mathcal{L}} p(\tilde{x},\tilde{y},\tilde{y}), & (\tilde{x}, \tilde{y}) \in \tilde{\Omega} \label{eq:qqq} \\
...
\begin{align}
  p(\tilde{x},\tilde{y},\tilde{t}) &=0,  \mbox{ for }  (\tilde{x},\tilde{y}) \in \partial\tilde{\Omega}, \nonumber \\
  p(\tilde{x},\tilde{y},0) &= \delta\left( \tilde{x} - \tilde{x}_0 \right)\delta\left( \tilde{y} - \tilde{y}_0 \right). \label{eq:qqq-2}
\end{align}
...
\[
  p(x,y,t) = p(\tilde{x}, \tilde{y}, \tilde{t}) \cdot \frac{1}{L_xL_y},
\]
such that the solution to the original problem \eqref{eq:1} -
\eqref{eq:2} can be expressed as the product
\[
  q(x,y,t) = \exp(-\alpha x - \beta y - \gamma t) \cdot p(\tilde{x}, \tilde{y}, \tilde{t}) \cdot \frac{1}{L_xL_y}.
\]
\begin{remark} \label{remark:diff}
  As a consequence of Remark \ref{remark:const-L}, all of the
  dependency of $q$ on $(a_x,b_x,a_y,b_y)$ is through the normalized
  solution $p(\tilde{x},\tilde{y},\tilde{t})$. Thus, without loss of
  generality, we will concentrate on the solution of the normalized
  problem (\ref{eq:qqq}).
\end{remark}

\subsection{Density Calculation} \label{sec:likelihood-calc} Recalling
problem (\ref{eq:pdf}) and Remark \ref{remark:diff}, to compute the
joint density $f(x,y,a_x,b_x,a_y,b_y)$ defined in \eqref{eq:pdf}, we
must take derivatives of the solution
...
\[
  p(\tilde{x},\tilde{y},\tilde{t} | a_x \pm \epsilon, b_x \pm \epsilon, a_y \pm \epsilon, b_y
  \pm \epsilon).
\]
...
c_{\{ k_1, k_2, k_3, k_4 \} } p(\left. \tilde{x},\tilde{y},\tilde{t} \right| a_x+k_1 \varepsilon, b_x+k_2 \varepsilon, 
...
$p^{(k)}(\left. \tilde{x},\tilde{y},\tilde{t} \right| b)$ where we
...
true value $\frac{\partial }{\partial b} p(\left. \tilde{x},\tilde{y},\tilde{t} \right| b)$
...
consequence of round-off errors and possible irregular truncation
...
eigenfunction expansion of $p(\tilde{x},\tilde{y},0)$. Because the differential
...
series expansion of $L \psi_{l, m}(\tilde{x},\tilde{y})$. In the case where
...
\[
  p(\tilde{x},\tilde{y},\tilde{t}) \approx \Psi(\tilde{x},\tilde{y})^T \sum_{\nu} h_\nu c_\nu e^{-\lambda_\nu t}.
\]

In terms of density calculations for
$\frac{\partial^4 p(\tilde{x},\tilde{y},\tilde{t})}{\partial a_x
  \partial b_x, \partial a_y \partial b_y}$, the eigenfunction
expansion does not suffer from having irregular truncation errors,
because $A$ is analytic in the diffusion parameters and the eigenpairs
$(\lambda_\nu, c_\nu)$ inherit this property (see Theorems 7 and 8 of
Chapter 9 in \cite{lax2007linear-algebra}). However, because a good
...
grid on $\tilde{\Omega}$ with $\Delta \tilde{x} = \Delta \tilde{y} = h$,
...
construct once and store the matrices $B_{\tilde{x},\tilde{x}}, B_{\tilde{x},\tilde{y}}, B_{\tilde{y},\tilde{y}}$,
...
for different parameter values $(\sigma_{\tilde{y}},\rho)$.
...
eigenvalue problem is therefore much cheaper to solve.
...
basic convergence estimate from approximation theory for semidiscrete
Galerkin-type solutions to parabolic problems.
...
$S_k$. Matrix $M$, matrix $S$, and vector $\bf{p}(0)$ are expressed in
elements as:
\begin{align*}
...
is differentiable with respect to the boundary parameters.
% Theorems 7 and 8 in
...
\subsection{Small-time Solution} \label{sec:pde-small-t} Before
...
by enforcing only one boundary condition, at the boundary that has the
largest influence on the solution. We start with the fundamental
solution $G_\rho(\tilde{x},\tilde{y},\tilde{t} | \tilde{x}_0, \tilde{y}_0)$ for the unbounded problem
in (\ref{eq:qqq}), which is the bivariate Gaussian density with mean
and covariance determined by the initial condition and the diffusion
parameters \cite{stakgold2011green}. We can then find a small enough
$\tilde{t}_\epsilon$ such that
$G_\rho\left(\tilde{x},\tilde{y}, \tilde{t}_\epsilon \left| \tilde{x}_0,
    \tilde{y}_0 \right.\right)$ is numerically zero on the boundaries
of $\tilde{\Omega}$. We select the largest among all time instances
satisfying this condition. This is done in the following way.
\begin{enumerate}
...
\item Reflect the point $(\xi_0, \eta_0) \to (\xi_0', \eta_0')$ about
  ...
  of $\Omega^{(3)}$. For this same reason, the system of images
  ...
  \begin{align}
    ...
    \MoveEqLeft[8] \frac{1}{2\pi\,\, \tilde{t}\sigma_{\tilde{y}}\sqrt{1-\rho^2}} \exp\left( -\frac{\left[ \left(\tilde{x}-\tilde{x}_0\right)^2 \sigma_{\tilde{y}}^2 + \left(\tilde{y}-\tilde{y}_0\right)^2 - 2\rho(\tilde{x}-\tilde{x}_0)(\tilde{y}-\tilde{y}_0)\sigma_{\tilde{y}} \right]}{2\,\,\tilde{t}\, \sigma_{\tilde{y}}^2 (1-\rho^2)} \right) \label{eq:Gfundamental}
    % \left[ \left(\tilde{x}-\tilde{x}_0\right)^2 \sigma_{\tilde{y}}^2 + \left(\tilde{y}-\tilde{y}_0\right)^2 - 2\rho(\tilde{x}-\tilde{x}_0)(\tilde{y}-\tilde{y}_0)\sigma_{\tilde{y}} \right]
    \end{align}
...
$\left\{(\lambda_j, \phi_j)\right\}$ over domain $\tilde{\Omega}$,
increasing absolute value of $\lambda_j$ with $j$ corresponds to
$\phi_j$ with increasing number of oscillations over
$\tilde{\Omega}$. The $\|\cdot\|_2$ norm is distict from the usual $L-2$ norm 
$L_2(\tilde{\Omega})$, which we denote as
...
will use an iterative derivative-free numerical algorithm to maximize
the likleihood which requires repeated evauluation of the likelihood
(the Nelder-Mead method; see \cite{lagarias1998convergence} for review
and convergence properties). For moderate to large samples sizes this
is feasible, because our numerical method is specifically designed for
...
to the true distribution $F(\cdot | \theta)$. Define the probability
densities
\begin{align}
  f(z) &= f(x,y,a_x,b_x,a_y,b_y) := \nonumber \\ 
\MoveEqLeft[5] \Pr\left(X(t) \in dx, Y(t) \in dy,
  m_x \in da_x, M_x \in db_x, m_y \in da_y, M_y \in
  db_y \left| \theta \right.\right), \label{eq:stong-density} \\
  f^{(k)}(z) &= \nonumber \\ \MoveEqLeft[5]
  f^{(k)}(x,y,a_x,b_x,a_y,b_y) := \frac{\partial^4}{\partial
  a_x \partial b_x \partial a_y \partial b_y}
  q^{(k)}(\tilde{x},\tilde{y},\tilde{t} | a_x, b_x, a_y,
  b_y), \label{eq:galerkin-density}
\end{align}
where we use the shorhand $X(t) \in dx$ for $X(t) \in (x, x+dx)$. The
function $f(z)$ is the probability density function corresponding to
$q(\tilde{x},\tilde{y},\tilde{t})$, the strong solution to the
governing Fokker-Planck equation; $f^{(k)}(z)$ corresponds to the
approximate solution $q^{(k)}$ of the unnormalized equation obtained
from the Galerkin approximation $p^{(k)}$ of the normalized
equation. Before proceeding, we should note that both $f(z)$ and
...
is bounded above by 1. Moreover, since $q(\tilde{x},\tilde{y},\tilde{t}|a_x,b_x,a_y,b_y)$ represents the
...
\[
  f(x,y,a_x,b_x,a_y,b_y) = \frac{\partial^4}{\partial a_x \partial b_x \partial a_y \partial
         b_y} q(\tilde{x},\tilde{y},\tilde{t} | a_x, b_x, a_y, b_y).
\]
...
$\left< \tilde{\mathcal{L}} \psi_i(\tilde{x},\tilde{y}), \psi_j(\tilde{x},\tilde{y}) \right>$ is
differentiable with respect to the parameters. The condition certainly
holds for the normalized problem (\ref{eq:qqq}) with our choice of
basis functions $S_k$, as they are infinitely differentiable with
repsect to $\tilde{x},\tilde{y}$, the boundaries, and the diffusion parameters. 
