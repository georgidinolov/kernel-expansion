...
trigonometric series expansion, inefficiencies arise from either using
a separable representation for the differential operator that is
...
Recalling equation (\ref{eq:pdf}), to compute the density we are
...
limitations of the type of approximation that can be used and the
...
\[
  ...
  \rightarrow 0
\]
What is even more severe, for a fourth-order derivative, the error
contribution of finite machine precision becomes
$O(\varepsilon_{mach}/\varepsilon^4)$ and a step size as big as
$\varepsilon \approx 10^{-4}$ produces errors $O(1)$. This is
...
that this is the case for the finite difference approximation.  As a
consequence of both machine precision and possible irregular
truncation errors, there is a fundamental restriction of the size of
finite difference steps $\varepsilon$ we can use in any numerical
method.
...
\] 
where $A$ is a constant matrix dependent $(\tau_x, \tau_y, \rho)$. In
the last part of the equation above, we have truncated the infinite
sine series expansion of $L \psi_{l, m}(x, y)$. In the case where
...
having irregular truncation errors. However, because a good working
...
method is that it introduces irregular truncation error. Referring to
the notation in (\ref{eq:trunc-error}) with $k := 1/h$, the
interpolation error introduced by a finite difference scheme at
position $b$ for the boundary parameters using step $h$ has the general form of
