...
\end{align}
This density, which we shorten to $q(x,y,t)$ from now on, is the
...
small-time analytic solution with a finite-element method. We apply
our computational method for finding (\ref{eq:CDF}) and (\ref{eq:pdf})
to estimate the equation parameters in settings where the
model assumption is appropriate. application is the maximal likelihood
estimation. 
