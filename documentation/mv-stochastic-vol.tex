...
\item
  ...
  We change the form of the basis functions in the Galerkin
...
distribution of the normalized parameters. To do so, we sample the diffusion
parameters $\sigma_x$ and $\sigma_y$ independently from a heavy-tailed
distribution in order to capture outlier. For our purposes, we sample
\begin{align}
...
(\ref{fig:small-sigma-y-t-samples}). Across three regimes of $\rho$,
extreme values for the normalized parameters are defined by having
either the smallest $\tau_y/\tau_x$ or $\tilde{t}$ in the sample
produced. These points are denoted by the the red and green sample
...
extreme values and may be most problematic with respect to our
Galerkin solution. An intersting observation is that there is a
tradeoff between the size of $\tau_y/\tau_x$ and $\tilde{t}$. In other
words, we will never encounter data points generated by a diffusion
equation having $(\tau_y/\tau_x, \tilde{t})$ in the lower-left corner
of the parameter space.

The quality of the Galerkin solution in the $\rho \neq 0$ cases cannot
be directly assessed because of a lack of closed-form analytic
expression. However, this is not the case for $\rho=0$. Figure
...
\begin{align}
  \tilde{\sigma}_x &= 0.3 &\tilde{\sigma}_y &= 0.07 &\tilde{\rho} &= 0.0,
\end{align}
...
(\ref{fig:small-sigma-y-t-samples}). Finally, Figure
(\ref{fig:rho-09}) shows the likelihood surfaces for the green and red
points where $\rho = 0.9$. The solution generated has properties
consisent with those of the analytic $\rho=0$ solution: conditional on
either $\tau_y/\tau_x$ or $\tilde{t}$, the solution formed, when
valid, exibits unimodality and constant concavity around the mode.
...
\begin{figure}
  \centering
  \includegraphics[scale=0.5]{small-sigma-Galerkin-no-filter-rho-09.pdf}
  \caption{.}
  \label{fig:rho-09}
\end{figure}
...
the solution does explicitly fail in a lot of instances. To evaluate
the likelihood in such conditions, we introduce an analytic extension
of our numerical solution.

\item To motivate the analytic correction to our numerical method, we
  consider the analytic solution for the problem when
  $\tilde{t} << 1$. We only modify the small-time solution in the
  previous chapter by considering a system of images defined not by a
  single reflection, but by a reflection about each of the four
  boundaries. This produces a solution having the form 
  \begin{align}
    p(x,y,\tilde{t}_\varepsilon) &= G(x,y | \mu, \Sigma) + \sum_{k}^K
    \omega_k G(x,y | \mu_k(a_x,b_x,a_y,b_y), \Sigma),
  \end{align}
  where $G(\cdot)$ is the Gaussian distribution and $K=8$. We see that
  when differentiated with respect to the boundaries, the small-time
  form for likelihood is a sum of Gaussians so that the likelihood
  proportional to either $\tilde{t}$ or $\tau_y/\tau_x$ has an
  inverge-Gamma form. The derivatives of the $\mu_k$ functions are
  constant with respect to the diffusion parameters and are of the
