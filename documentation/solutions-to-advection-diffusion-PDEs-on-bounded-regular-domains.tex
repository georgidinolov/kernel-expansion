...
way. Consider a countable family of functions
...
\begin{align*}
  ...
  \frac{1}{2}\sigma^2 \displaystyle \int_\Omega \partial^2_x q_{weak}(x,t) \psi_k(x)\,dx = 0.
\end{align*}

It should be noted that each element $\psi_k(x)$
need not be $C^\infty_0(\Omega)$. In fact, under the Ritz method,
$\psi_k(x) \in C^1_0(\Omega)$ in the weak sense.

In application, we restrict ourselves to a subset of $\{\psi_k(x)\}_{k=1}^\infty$:
\[
  \{\psi_k(x)\}_{k=1}^K.
\]
